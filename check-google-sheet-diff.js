...
async function run() {
    console.log('📡 Descargando Google Sheet en vivo...');
    const rawCSV = await fetchCSV(SHEET_URL);
    const sheetLines = rawCSV.split(/\r\n|\n/);

    const localLines = fs.readFileSync(CSV_PATH, 'utf8').split(/\r\n|\n/);
    ...
    console.log(`📊 Registros en atenciones.csv local: ${localLines.length}`);
    console.log(`📊 Líneas en Google Sheet en vivo: ${sheetLines.length}`);
    ...
    console.log(`\n🔍 Nuevos registros REALES con datos encontrados en Google Sheet: ${newRows.length}`);
    newRows.forEach((r, idx) => {
        console.log(`[${idx+1}] Fecha: ${r.fecha} | DNI: ${r.dni} | Nombre: ${r.apellidos} ${r.nombres} | Expte/Motivo: ${r.expte || r.motivo} | Defensoria: ${r.defensoria} | Operador: ${r.atendidoPor}`);
    });
}
